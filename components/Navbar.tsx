...
interface NavbarProps {
  ...
  onStudentPortal?: () => void; // Made optional with ?
}
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled 
        ...
        : 'py-4 bg-gradient-to-b from-slate-950/90 to-transparent'
    }`}>
  ...
  {logoError && (
    <div className="absolute inset-0 flex items-center justify-center bg-red-600 rounded-lg">
      <span className="text-white font-bold text-lg">BTC</span>
        ...
        <div className="hidden lg:flex items-center space-x-8">
          ...
          {onStudentPortal && (
            <button 
              ...
              className="text-xs font-bold text-blue-400 hover:text-white transition-colors uppercase tracking-widest"
            >
              Student Portal
            </button>
          )}
          <button 
            ...
            className="relative px-6 py-2.5 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 text-white rounded-full font-bold transition-all transform hover:scale-105 shadow-lg shadow-red-500/25 active:scale-95 group overflow-hidden"
          >
        ...
        {/* Mobile Menu Toggle */}
        <button 
          className="lg:hidden p-2 text-white hover:text-red-400 transition-colors relative z-50"
          onClick={(e) => {
            e.stopPropagation();
      ...
      {isMobileMenuOpen && (
        <div 
          className="lg:hidden fixed inset-0 bg-slate-950/95 backdrop-blur-sm pt-24 z-40 animate-fadeIn"
            ...
            <div className="space-y-2 animate-slideDown">
              {menuItems.map((item) => (
                <button
                  key={item.page}
                  onClick={() => handleNavClick(item.page)}
                  className={`block w-full text-left px-6 py-4 rounded-xl text-lg font-semibold transition-all duration-300 ${
                    currentPage === item.page 
              ...
              {onStudentPortal && (
                <button 
                  onClick={onStudentPortal}
                  className="block w-full text-left px-6 py-4 rounded-xl text-lg font-bold bg-gradient-to-r from-blue-600 to-blue-500 text-white hover:from-blue-700 hover:to-blue-600 transition-all duration-300"
                >
                  Student Portal
                </button>
              )}
              <button 
                onClick={() => handleNavClick(Page.Apply)}
                className="block w-full text-left px-6 py-4 rounded-xl text-lg font-bold bg-gradient-to-r from-red-600 to-red-500 text-white hover:from-red-700 hover:to-red-600 transition-all duration-300 transform hover:scale-[1.02] active:scale-95"
              >
                APPLY NOW
              </button>
              ...
              <div className="mt-8 pt-8 border-t border-white/10">
                <div className="space-y-4 text-gray-400">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <span>info@bathuditraining.co.za</span>
                  </div>
                </div>
              </div>
            </div>
